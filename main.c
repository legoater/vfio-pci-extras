...
static void dvsec_set_buf_addr(struct igb_pci_core_device *dev, phys_addr_t addr)
{
...
static int dvsec_read(struct pci_dev *pdev, int pos, u32 *val)
{
	int ret;

	ret = pci_read_config_dword(pdev, pos, val);
	if (ret)
...
static int igb_get_state(struct pci_dev *pdev, int pos, u32 *state)
{
	u32 status;
	int ret;

	ret = dvsec_read(pdev, pos + IGB_MIG_STATUS, &status);
	if (ret)
		return ret;
...
static int dvsec_send_cmd(struct igb_pci_core_device *dev, u32 cmd)
{
	...
	ret = dvsec_read(dev->pdev, dev->dvsec_pos + IGB_MIG_STATUS, &status);
	if (ret) {
...
static int igb_validate_dvsec_header(struct pci_dev *pdev, int pos)
{
	u32 hdr;
	int ret;

	ret = dvsec_read(pdev, pos + PCI_DVSEC_HEADER1, &hdr);
	if (ret)
		return ret;
			      ...
			      struct igb_pci_core_device *igb_dev)
{
	...
	ret = igb_validate_dvsec_header(pdev, pos);
	if (ret)
		return ret;

	ret = dvsec_read(pdev, pos + IGB_MIG_CAPS, &caps);
	if (ret)
		return ret;
	...
	ret = dvsec_read(pdev, pos + IGB_MIG_DATA_SIZE,
	...
	/* Verify the device is in the expected initial state */
	ret = igb_get_state(pdev, pos, &state);
	if (ret)
		return ret;
	if (state != IGB_MIG_STATE_RUNNING) {
		dev_err(&pdev->dev, "unexpected initial state %u\n", state);
		return -EINVAL;
	}
				  ...
				  struct igb_migration_file *migf)
{
	...
	ret = dvsec_read(pdev, igb_dev->dvsec_pos + IGB_MIG_DATA_SIZE,
			 &data_size);
	if (ret)
		return ret;
...
static int igb_pci_open_device(struct vfio_device *core_vdev)
{
	...
	/* Verify the device is in the expected initial state */
	ret = igb_get_state(igb_dev->pdev, igb_dev->dvsec_pos, &state);
	if (ret) {
				...
				u64 *page_size)
{
	...
	*page_size = max_t(u64, *page_size, IGB_MIG_DIRTY_DEFAULT_PGSIZE);
	...
	for (i = 0; i < num_ranges && node; i++) {
		...
		dvsec_set_buf_addr(igb_dev, virt_to_phys(req));
...
static int igb_stats_show(struct seq_file *s, void *unused)
{
	...
	dvsec_set_buf_addr(igb_dev, virt_to_phys(stats));
...
static int igb_vfio_pci_init_dev(struct vfio_device *core_vdev)
{
	...
	ret = igb_discover_dvsec(pdev, igb_dev);
	if (ret)
		return ret;
